...
function makeReqInvitationsPOST(cookies, invitee) {
  ...
  const headers = _extends({}, constants.headers.normInvitationsPOST, {
...
function printInviteCard(invitee, isSuccess, successCount, failedCount) {
  ...
  const wrappedInvOccupation = utils.wrapText(utils.resolveNewLines(invitee.occupation), wrapOption);
